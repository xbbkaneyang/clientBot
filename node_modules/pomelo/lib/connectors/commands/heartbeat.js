...
var peerLogger = require('pomelo-logger').getLogger('peer');
...
Command.prototype.handle = function(socket) {
  ...
  if(self.disconnectOnTimeout) {
    self.timeouts[socket.id] = setTimeout(function() {
      ...
      peerLogger.warn('peer heartbeat timeout : %s-%s' , socket.id , socket.remoteAddress.ip );
      if ( socket && socket.socket ) {
        socket.socket.emit('close', 'heartbeat', '');
        socket.socket.close();
      } else {
        socket.disconnect();
      }
