...
pomelo.prototype.onKick = function(self,data) {
...
pomelo.prototype.initWebSocket = function(url,cb){
  ...
  var onopen = function(event){
      // console.info( self.socket );
    self.handshakeBuffer.sys.protoVersion = protoVersion;
...
pomelo.prototype.disconnect = function() {
    console.error("Call pomelo.prototype.disconnect");
  if(this.socket) {
...
pomelo.prototype.heartbeatTimeoutCb = function() {
  ...
  if(gap > self.gapThreshold) {
    self.heartbeatTimeoutId = setTimeout(self.heartbeatTimeoutCb.bind(self), gap);
  } else {
      console.log( "heartbeat timeoutheartbeat timeout" );
...
var send = function(self,packet) {
  if (!!self.socket) {
      self.socket.send( packet, {binary: true, mask: true});
